...
struct PendingInviteCard: View {
    ...
    var body: some View {
        if let image = profile.image, let event = profile.event {
            Button {
                withAnimation(nil) {
...
//                        .defaultImage(132)
                ...
                .overlay(alignment: .bottomTrailing) {
                    HStack(spacing: 4) {
                        ...
                        SimpleClockView(targetTime: event.inviteExpiryTime) {}
                    }
            ...
            .buttonStyle(.plain)   // avoids the default blue button styling
        }
    }
}
