...
//
 ...
 //  2. apply `.profileMorphHost(morph)` to the ZStack that presents ProfileContainer
 //  3. drive the presented ProfileContainer with `.opacity(morph.contentOpacity)` (no
 ...
 //ProfileContainer picks the state up from the environment and routes every dismiss
 ...
 final class ProfileMorphState {
     ...
     static let cardRadii = RectangleCornerRadii(uniform: CornerRadius.concentric(in: CornerRadius.md, inset: 6)) //Concentric inside the 16pt event card
     static let pagerRadii = RectangleCornerRadii(uniform: CornerRadius.image) //Must match the profile pager's image clip
 ...
 struct ProfileZoomDismissModifier: ViewModifier {
     ...
     func body(content: Content) -> some View {
         //Disabled means another system owns this profile's dismissal (own
         //profile, or a zoom presentation — \.zoomPresented). The render must
         //then be fully absent, not just drag-zeroed: its rest-state clip
         //tracks the SCREEN in unscaled local units, so any outside transform
         //(the zoom library's flight) moves the global frame and the clip
         //bites the content's trailing edge — a white band over the image.
         if !enabled {
             content
         } else {
             content
                 .modifier(ZoomDismissRender(
                     dragX: ui.profileOffsetX,
                     dragY: max(ui.profileOffset, 0),
 ...
 extension View {
     ...
     //Injects the morph for this subtree (sources, pager, ProfileContainer's dismiss all
