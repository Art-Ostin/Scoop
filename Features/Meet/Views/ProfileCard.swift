...
/*
 ...
 private var detailsBackedBlur: some View {
     Image(uiImage: profile.image)
         .resizable()
         .scaledToFill()
         .frame(width: size, height: size)
         .blur(radius: 22)
         .mask(detailsBlurMask)
         .clipShape(RoundedRectangle(cornerRadius: cardCornerRadius))
         .allowsHitTesting(false)
 }
 
 
 private var nameBackgroundBlur: some View {
     Image(uiImage: profile.image)
         .resizable()
         .scaledToFill()
         .frame(width: size, height: size)
 ...
 private var nameBlurMask: some View {
     ...
     let padY: CGFloat = 2
     let feather: CGFloat = 4
     let rect = nameFrame.insetBy(dx: -padX, dy: -padY)
     return RoundedRectangle(cornerRadius: 10)
         .frame(width: max(rect.width, 0), height: max(rect.height, 0))
         .position(x: rect.midX, y: rect.midY)
         .blur(radius: feather)
         .opacity(nameFrame == .zero ? 0 : 1)
 }
 */
