...
extension PassionsView {
    ...
    private var titleSection: some View {
        titleView(text: "Passions", count: 3, subtitle: "(max 5)")
            .padding(.top, 60)
            .padding(.bottom, 60)
    }
        ...
        FlowLayout(mode: .scrollable, items: socialPassions, itemSpacing: 6) { item in
            Button {
                if let index = selectedPassions.firstIndex(of: item) {
                    selectedPassions.remove(at: index) // Deselect if already selected
                } else {
                    selectedPassions.append(item) // Select if not selected
                }
            } label: {
                Text(item)
