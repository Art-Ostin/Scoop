...
class EventManager {
    ...
    private func eventsQuery(_ scope: EventScope, userId: String, now: Date = .init()) throws -> Query {
    ...
    private func getEvents(_ scope: EventScope, userId: String, now: Date = .init()) async throws -> [UserEvent] {
        let query = try eventsQuery(scope, userId: userId, now: now)
