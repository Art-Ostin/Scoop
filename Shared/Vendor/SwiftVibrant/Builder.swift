...
public class Builder {
    ...
    public func addFilter(_ f: VibrantFilter)->Builder {
            self._opts.filters.append(f)
            return self
        }

    public func removeFilter(_ f: VibrantFilter)->Builder {
        self._opts.filters.removeAll(where: { (callback: VibrantFilter) in
            callback.id == f.id
        })
        return self
    }
