...
- Motion: only named roles from the `extension Animation` in
  ...
  updates), `.handOff` (a flown copy dissolving over the identical view it just landed on), `.sprout`/`.tuck` (chrome springing out of the control that owns it and back in — the one pair that overshoots on purpose; drive a single progress with it and pose every piece from that value). Call sites read `.animation(.toggle, value:)` / `withAnimation(.move) { }`.
