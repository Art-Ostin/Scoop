...
- Motion: only named roles from the `extension Animation` in
  ...
  updates), `.vanish` (~20ms: a control leaving with the tap that ended its job — the note's Done; a fade, never a cut), `.handOff` (a flown copy dissolving over the identical view it just landed on), `.keyboard` (content riding the system keyboard in or out — the keyboard's own spring, measured; everything a focus moves leaves in ONE transaction on it, and it never bounces), `.followThroughRise`/`.followThroughSettle` (what a `.keyboard` ride carries landing a few points past its place and easing back — a pair driving one 0 → 1 → 0 value, the settle issued from the rise's completion), `.sprout`/`.tuck` (chrome springing out of the control that owns it and back in — the one pair that overshoots on purpose; drive a single progress with it and pose every piece from that value). Call sites read `.animation(.toggle, value:)` / `withAnimation(.move) { }`.
