...
class ProfileRepo: ProfilesRepository {
    ...
    func profilesTracker(userId: String) async throws -> AsyncThrowingStream<FSCollectionEvent<UserEvent>, Error> {
        let profilesPath = "users/\(userId)/profiles"
        
        return fs.streamCollection(userEventsPath) {$0.whereField(ProfileRec.Status.rawValue, isEqualTo: .pending)}
